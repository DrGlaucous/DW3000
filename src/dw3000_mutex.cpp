...
// user-defined mutexes, we use the arduino backend for these
// this file uses the same header as dw3000_mutex.cpp
...
DW3000::decaIrqStatus_t DW3000::decamutexon(void)
{
	decaIrqStatus_t s = port->port_GetEXT_IRQStatus();
 ...
 * returns the state of the DW1000 interrupt
 */
void DW3000::decamutexoff(decaIrqStatus_t s) // put a function here that re-enables the interrupt at the end of the critical section
{
	if (s)
	{
		port->port_EnableEXT_IRQ();
	}
