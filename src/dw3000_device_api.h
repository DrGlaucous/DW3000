...
class DW3000
{
	...
	*/
	...
	// this can be static because it'll be the same across instances of the class
	static uint8_t crcTable[256];

	DWUart* uart;// = DWUart(115200);
	DW3000Port* port;// = DW3000Port();

public:


	DW3000(DWUart* uart_in, DW3000Port* port_in) {
		uart = uart_in;
		port = port_in;
	}
	...
	void dwt_xfer3000(uint32_t regFileID, // 0x0, 0x04-0x7F ; 0x10000, 0x10004, 0x10008-0x1007F; 0x20000 etc
	 ...
	 */
	int dwt_configure(const dwt_config_t *config);
	 ...
	 * no return value
	 */
	void dwt_configuretxrf(const dwt_txconfig_t *config);
	 ...
	 * returns DWT_SUCCESS for success, or DWT_ERROR for error
	 */
	int dwt_writetxdata(uint16_t txDataLength, const uint8_t *txDataBytes, uint16_t txBufferOffset);
	...
	{
		port->wakeup_device_with_io();
	}

	/*! ------------------------------------------------------------------------------------------------------------------
	 * @brief  This function wakeup device by an IO pin. It calls the method above.
	 *
	 * @param None
	 *
	 * output parameters
	 ...
	 * returns DWT_SUCCESS for success, or DWT_ERROR for error
	 */
	// not implemented or used by this library, so we don't need it here
	...
	{
		return port->writetospi(headerLength, headerBuffer, bodylength, bodyBuffer);
	}
	...
	{
		return port->readfromspi(headerLength, headerBuffer, readlength, readBuffer);
	}
	...
	void deca_usleep(uint8_t time_us) {
		port->deca_usleep(time_us);
	}

	/////////////////////////////////////////////////////////////////////////////////////////
	// these are implemented in the corresponding api cpp file
	...
	/* MACROS */
	void dwt_write32bitreg(uint32_t addr, uint32_t value)
	{
		dwt_write32bitoffsetreg(addr, 0, value);
	}

	uint32_t dwt_read32bitreg(uint32_t addr) { return dwt_read32bitoffsetreg(addr, 0); }
	void dwt_writefastCMD(uint32_t cmd) { dwt_writetodevice(cmd, 0, 0, 0); }

	//makes setting and resetting registers a little easier
	void dwt_or8bitoffsetreg(uint32_t addr, uint16_t offset, uint8_t or_val) { dwt_modify8bitoffsetreg(addr, offset, -1, or_val); }
	void dwt_and8bitoffsetreg(uint32_t addr, uint16_t offset, uint8_t and_val) { dwt_modify8bitoffsetreg(addr, offset, and_val, 0); }
	void dwt_and_or8bitoffsetreg(uint32_t addr, uint16_t offset, uint8_t and_val, uint8_t or_val) { dwt_modify8bitoffsetreg(addr, offset, and_val, or_val); }
	void dwt_set_bit_num_8bit_reg(uint32_t addr, uint8_t bit_num) { dwt_modify8bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num)); }
	void dwt_clr_bit_num_8bit_reg(uint32_t addr, uint8_t bit_num) { dwt_modify8bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0); }

	void dwt_or16bitoffsetreg(uint32_t addr, uint16_t offset, uint16_t or_val) { dwt_modify16bitoffsetreg(addr, offset, -1, or_val); }
	void dwt_and16bitoffsetreg(uint32_t addr, uint16_t offset, uint16_t and_val) { dwt_modify16bitoffsetreg(addr, offset, and_val, 0); }
	void dwt_and_or16bitoffsetreg(uint32_t addr, uint16_t offset, uint16_t and_val, uint16_t or_val) { dwt_modify16bitoffsetreg(addr, offset, and_val, or_val); }
	void dwt_set_bit_num_16bit_reg(uint32_t addr, uint16_t bit_num) { dwt_modify16bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num)); }
	void dwt_clr_bit_num_16bit_reg(uint32_t addr, uint16_t bit_num) { dwt_modify16bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0); }

	void dwt_or32bitoffsetreg(uint32_t addr, uint16_t offset, uint32_t or_val) { dwt_modify32bitoffsetreg(addr, offset, -1, or_val); }
	void dwt_and32bitoffsetreg(uint32_t addr, uint16_t offset, uint32_t and_val) { dwt_modify32bitoffsetreg(addr, offset, and_val, 0); }
	void dwt_and_or32bitoffsetreg(uint32_t addr, uint16_t offset, uint32_t and_val, uint32_t or_val) { dwt_modify32bitoffsetreg(addr, offset, and_val, or_val); }
	void dwt_set_bit_num_32bit_reg(uint32_t addr, uint32_t bit_num) { dwt_modify32bitoffsetreg(addr, 0, -1, DWT_BIT_MASK(bit_num)); }
	void dwt_clr_bit_num_32bit_reg(uint32_t addr, uint32_t bit_num) { dwt_modify32bitoffsetreg(addr, 0, ~DWT_BIT_MASK(bit_num), 0); }


	///////////////////////////////////////////////
	//my custom methods

	//set GPIO 0 through 8 as output
	void gpio_init_output() {
		dwt_and8bitoffsetreg(GPIO_DIR_ID, 0, 0x00);
	}

	//pull one of the GPIO pins high or low
	void gpio_set(uint8_t pin, uint32_t value) {
		pin %= 8;
		if(value) {
			dwt_set_bit_num_8bit_reg(GPIO_OUT_ID, pin);
		} else {
			dwt_clr_bit_num_8bit_reg(GPIO_OUT_ID, pin);
		}
	}
	...
	int check_for_rx() {
		...
		//get current status
		int sys_stat = dwt_read32bitreg(SYS_STATUS_ID);
	...
	//writes a 1 to all the status registers to clear them (since status stuff latches and doesn't go away when read; it must be written to)
	void clear_system_status() {
		//the bitmask, all the 0s are reserved addresses, see page 93
		uint32_t reg_bm = 0b00111111101111111111111111111111;

		//clear them
		dwt_write32bitreg(SYS_STATUS_ID, reg_bm);
	}
	
	//return the length of the last gotten frame in the RX buffer
	uint32_t get_frame_length() {
		return dwt_read32bitreg(RX_FINFO_ID) & RX_FINFO_RXFLEN_BIT_MASK;
	}

	//returns true if the frame was sent out successfully, returns false if it wasn't or is still sending
	bool check_frame_tx_success() {
		uint32_t system_status = dwt_read32bitreg(SYS_STATUS_ID);
		if(system_status & SYS_STATUS_TXFRS_BIT_MASK) {
			//send success
			return true;
		}
		//send failure or still sending
		return false;
	}

	//check a series of registers for flags that indicate that the module has stabilized
	bool check_for_idle() {

		//see page 216 of the dw3000 manual, we collect the 3rd byte (0 indexed), which contains the pmsc state
		auto pmsc_state = dwt_read8bitoffsetreg(SYS_STATE_LO_ID, 2);

		//see page 93, we collect the whole status register since we have a bit mask for it
		auto sys_status = dwt_read32bitreg(SYS_STATUS_ID);

		//apply masks
		bool pmsc_ready = !!(pmsc_state & PMSC_STATE_IDLE); //I wish I knew what official bit mask this was
		bool sys_status_ready = !!(sys_status & (SYS_STATUS_SPIRDY_BIT_MASK | SYS_STATUS_RCINIT_BIT_MASK));

		//return if both are true
		return pmsc_ready && sys_status_ready;

	}

	//stolen from shared_functions.cpp, is an endian-independant way to read the u64 timestamps
	...
	{
		uint32_t time = dwt_readsystimestamphi32();
		uint64_t ts = (uint64_t)time << 8;
		return ts;
	}
