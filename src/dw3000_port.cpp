...
void DW3000Port::enableDebounceClock()
{
...
void DW3000Port::sleepms(uint32_t x)
{
	delay(x);
}

int DW3000Port::sleepus(uint32_t x)
{
	delayMicroseconds(x);
	return 0;
}

void DW3000Port::deca_sleep(uint8_t time_ms) // wrapper for decawave sleep function
{
	sleepms(time_ms);
}

void DW3000Port::deca_usleep(uint8_t time_us) // wrapper for decawave sleep function
{
	sleepus(time_us);
}

void DW3000Port::spiBegin(uint8_t irq, uint8_t rst)
{
	...
	*/
	...
	spiInterface->setPins(DW3000_PIN_MISO, DW3000_PIN_SCK, DW3000_PIN_MOSI);
#endif
	spiInterface->begin();
#ifndef ESP8266
//  spiInterface->usingInterrupt(digitalPinToInterrupt(irq)); // not every board support this, e.g. ESP8266
#endif
...
void DW3000Port::reselect(uint8_t ss)
{
	_ss = ss;
	pinMode(_ss, OUTPUT);
...
void DW3000Port::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n)
{
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
...
void DW3000Port::readBytesOTP(uint16_t address, byte data[])
{
...
void DW3000Port::writeByte(byte cmd, uint16_t offset, byte data)
{
...
void DW3000Port::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size)
{
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
	...
	spiInterface->beginTransaction(*_currentSPI);
	digitalWrite(_ss, LOW);
	for (i = 0; i < headerLen; i++)
	{
		spiInterface->transfer(header[i]); // send header
	}
	for (i = 0; i < data_size; i++)
	{
		spiInterface->transfer(data[i]); // write values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
...
//change the speed of SPI and the expected speed by the radio
void DW3000Port::enableClock(uint8_t clock)
{
...
void DW3000Port::setBit(byte data[], uint16_t n, uint16_t bit, boolean val)
{
	uint16_t idx;
	uint8_t shift;
 ...
 */
boolean DW3000Port::getBit(byte data[], uint16_t n, uint16_t bit)
{
	uint16_t idx;
	uint8_t shift;
...
void DW3000Port::writeValueToBytes(byte data[], int32_t val, uint16_t n)
{
	uint16_t i;
	for (i = 0; i < n; i++)
	{
...
void DW3000Port::setDoubleBuffering(boolean val)
{
	setBit(_syscfg, LEN_SYS_CFG, DIS_DRXB_BIT, !val);
}

void DW3000Port::setInterruptPolarity(boolean val)
{
	setBit(_syscfg, LEN_SYS_CFG, HIRQ_POL_BIT, val);
}

void DW3000Port::clearInterrupts()
{
	memset(_sysmask, 0, LEN_SYS_MASK);
}

void DW3000Port::manageLDE()
{
...
void DW3000Port::Sleep(uint32_t d)
{
	delay(d);
}

void DW3000Port::spiSelect(uint8_t ss)
{
	reselect(ss);
...
int DW3000Port::readfromspi(uint16_t headerLength, const uint8_t *headerBuffer, uint16_t readLength, uint8_t *readBuffer)
{
	...
	{
		spiInterface->transfer(headerBuffer[i]); // send header
	}
	for (int i = 0; i < readLength; i++)
	{
		readBuffer[i] = spiInterface->transfer(JUNK); // read values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	spiInterface->endTransaction();
...
int DW3000Port::writetospi(uint16_t headerLength, const uint8_t *headerBuffer, uint16_t bodyLength, const uint8_t *bodyBuffer)
{
	spiInterface->beginTransaction(*_currentSPI);
	digitalWrite(_ss, LOW);
	for (int i = 0; i < headerLength; i++)
	...
	{
		spiInterface->transfer(bodyBuffer[i]); // write values
	}
	delayMicroseconds(5);
	digitalWrite(_ss, HIGH);
	spiInterface->endTransaction();
...
void DW3000Port::wakeup_device_with_io()
{
	digitalWrite(_ss, LOW);
	delay(2);
	digitalWrite(_ss, HIGH);
...
uint32_t DW3000Port::port_GetEXT_IRQStatus(void)
{
	return _irq_enabled ? 1UL : 0UL;
}

uint32_t DW3000Port::port_CheckEXT_IRQ(void)
{
	return digitalRead(_irq) ? 1UL : 0UL;
}

void DW3000Port::port_DisableEXT_IRQ(void)
{
	_irq_enabled = false;
}

void DW3000Port::port_EnableEXT_IRQ(void)
{
	_irq_enabled = true;
}
 ...
 * @return none
 */
void DW3000Port::port_set_dwic_isr(port_dwic_isr_t dwic_isr)
{
