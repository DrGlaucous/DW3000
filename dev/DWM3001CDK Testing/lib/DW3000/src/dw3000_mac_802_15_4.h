 ...
 *
 */
// these frames are used to talk to apple devices n such, and are not strictly required for doing ranging stuff
#pragma once
...
class MacFrame802_15_4
{
	...
	mac_frame_802_15_4_format_t *mac_frame_ptr = &mac_frame;

public:
	void mac_frame_init_mac_frame_ctrl();
	void mac_frame_set_pan_ids_and_addresses_802_15_4(uint16_t dest_pan_id, uint64_t dest_addr /*,uint16_t src_pan_id*/, uint64_t src_addr);
	void mac_frame_update_sequence_number(uint8_t seq_num);
	void mac_frame_update_aux_frame_cnt(uint32_t frame_cnt);
	void mac_frame_set_aux_key_identifier(uint8_t key_index);
	uint8_t mac_frame_get_aux_key_identifier();
	uint32_t mac_frame_get_aux_frame_cnt();
	void mac_frame_get_nonce(uint8_t *aes_iv);
	void mac_frame_set_aux_security_control();
	uint8_t mac_frame_get_aux_mic_size();
	...
	security_state_e get_security_state();
	void get_src_and_dst_frame_addr(uint64_t *src, uint64_t *dst);
